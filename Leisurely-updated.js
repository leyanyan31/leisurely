...
function wireDashboard() {
    ...
    document.querySelectorAll(".edit-task-btn").forEach((btn) => {
        btn.onclick = () => {
            ...
            document.getElementById("editRoomOptions").innerHTML = `<option value="">Personal</option>${user.rooms.map((r) => `<option value="${r.id}" ${r.id === task.roomId ? 'selected' : ''}>${esc(r.name)}</option>`).join("")}`;
            editModal.classList.remove("hidden");
            editModal.setAttribute("aria-hidden", "false");
        };
    });
    document.getElementById("closeEditTaskModalBtn").onclick = () => {
