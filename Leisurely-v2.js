...
function wireDashboard() {
    ...
    document.getElementById("openTaskModalBtn").onclick = () => {
        ...
        delete taskModal.dataset.editId;
    ...
    document.getElementById("taskForm").onsubmit = (e) => {
        ...
        const editId = taskModal.dataset.editId;
        
        if (editId) {
            const task = user.tasks.find(t => t.id === editId);
            if (task) {
    ...
    const renderTasks = () => {
        const rows = roomTasks.map((t) => {
            ...
            return `<tr>
                ...
                <td>${esc(t.owner)}</td>
                <td>
                    ${t.ownerEmail === user.email ? `<button type="button" class="action-btn toggle-done-btn" data-task-id="${t.id}">${t.status === "done" ? "Undo" : "Done"}</button>
                    <button type="button" class="action-btn edit-task-btn" data-task-id="${t.id}">Edit</button>
...
function wireRooms() {
    ...
    roomList.innerHTML = user.rooms.map((r) => `
        <li class="mini-card room-card" role="button" tabindex="0" aria-label="Open room ${esc(r.name)}">
            <button type="button" class="room-card-btn" onclick="openRoom('${r.id}')">
                <strong>${esc(r.name)}</strong><br />
                <small>${r.members?.length || 1} members</small><br />
                <small style="font-family:monospace; letter-spacing:1px;">Code: ${r.code || 'N/A'}</small>
            </button>
            <button type="button" class="action-btn ghost" onclick="copyRoomCode('${r.code}')" aria-label="Copy room code">Copy Code</button>
